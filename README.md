...
```bash
git clone https://github.com/Tanishqarya17/Multi_Agent_Movie_Recommender.git
cd Multi_Agent_Movie_Recommender
```
...
Note on artifacts: Trained model files (ALS model, FAISS index, catalog) are not stored in this repository due to size. They are generated by running the notebooks in order, and the deployed application on Hugging Face Spaces includes them.
